  ...
  <div class="flex flex-col items-center gap-6 w-full max-w-sm mx-auto">
    ...
    <form @submit.prevent="handleSubmit" class="w-full flex flex-col gap-6">
      <div class="flex flex-col gap-2">
        <BaseInput v-model="form.password" :label="$t('auth.labels.newPassword')" type="password"
      ...
      <NuxtLink to="/login" class="text-sm font-medium text-center text-[#6C4EFD] hover:opacity-80 transition-opacity">
...
const handleSubmit = async () => {
  ...
  if (!token) {
    useToast().add({ title: 'Missing reset token', color: 'error' })
    return
  }
  ...
  if (!result.success && result.error) {
    useToast().add({ title: result.error || 'Reset failed', color: 'error' })
  } else {
    useToast().add({ title: t('auth.resetPassword.successDescription'), color: 'success' })
    navigateTo('/login')
  }
}
</script>
