<template>
  <div class="w-full max-w-[547px] flex flex-col gap-8">
    <!-- Header -->
    <div class="flex flex-col items-center gap-2 text-center mb-4">
      <h1 class="text-4xl font-bold text-white font-poppins">
        {{ $t('auth.login.welcome') }}
      </h1>
      <p class="text-base text-gray-400 font-poppins">
        {{ $t('auth.login.subtitle') }}
      </p>
    </div>

    <!-- Form -->
    <form @submit.prevent="handleSubmit" class="flex flex-col gap-6">
      <BaseInput v-model="form.email" :label="$t('auth.labels.email')" type="email"
        :placeholder="$t('auth.login.emailPlaceholder')" icon="ph:envelope-simple" :error="errors.email" forceDark />

      <div class="flex flex-col gap-2">
        <BaseInput v-model="form.password" :label="$t('auth.labels.password')" type="password"
          :placeholder="$t('auth.login.passwordPlaceholder')" icon="ph:lock-key" :error="errors.password" forceDark />

        <div class="flex justify-between items-center mt-1">
          <!-- Remember Me -->
          <div class="flex items-center gap-2">
            <input id="remember" v-model="form.rememberMe" type="checkbox"
              class="w-4 h-4 rounded border-gray-600 bg-gray-800 text-[#6C4EFD] focus:ring-[#6C4EFD]" />
            <label for="remember" class="text-sm text-gray-300 font-poppins">
              {{ $t('auth.login.rememberMe') }}
            </label>
          </div>

          <!-- Forgot Password -->
          <NuxtLink to="/forgot-password"
            class="text-sm font-poppins font-semibold text-white hover:text-gray-300 transition-colors">
            {{ $t('auth.login.forgotPassword') }}
          </NuxtLink>
        </div>
      </div>

      <button type="submit" :disabled="isLoading"
        class="w-full py-4 mt-2 text-white font-semibold rounded-xl bg-primary-600 hover:bg-primary-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-base font-poppins">
        <span v-if="isLoading">{{ $t('auth.login.signingIn') }}</span>
        <span v-else>{{ $t('auth.login.submit') }}</span>
      </button>
    </form>

    <!-- Footer -->
    <div class="flex flex-col items-center gap-6 mt-2">
      <div class="text-center">
        <span class="text-gray-400 font-poppins">{{ $t('auth.login.noAccount') }} </span>
        <NuxtLink to="/register"
          class="font-semibold font-poppins text-white hover:text-gray-300 transition-colors">
          {{ $t('auth.login.signUp') }}
        </NuxtLink>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { loginSchema } from '~/schema/auth'

const { t } = useI18n()
const toast = useToast()
const route = useRoute()
const { login, isLoading, error: authError } = useAuth()

const form = reactive({
  email: '',
  password: '',
  rememberMe: false
})

const errors = reactive<Record<string, string>>({})

const handleSubmit = async () => {
  // Clear errors
  Object.keys(errors).forEach(key => delete errors[key])

  const validationResult = loginSchema.safeParse(form)

  if (!validationResult.success) {
    validationResult.error.issues.forEach(issue => {
      const field = issue.path[0] as string
      errors[field] = issue.message
    })
    return
  }

  try {
    const result = await login(validationResult.data)

    if (!result.success && result.error) {
      toast.add({
        title: t('auth.login.error'),
        description: result.error,
        color: 'error'
      })
    } else {
      toast.add({
        title: t('auth.login.success'),
        color: 'success'
      })
      const redirectTo = (route.query.redirect as string) || '/dashboard'
      navigateTo(redirectTo)
    }

  } catch (err) {
    console.error('Login error:', err)
  }
}
</script>
