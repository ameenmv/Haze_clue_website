export default {
  labels: {
    name: 'الاسم',
    firstName: 'الاسم الأول',
    lastName: 'اسم العائلة',
    email: 'البريد الإلكتروني',
    phone: 'رقم الجوال',
    message: 'الرسالة',
    country: 'الدولة',
    organization: 'المنظمة',
    messageSubject: 'موضوع الرسالة',
    userName: 'اسم المستخدم',
    password: 'كلمة المرور',
    newPassword: 'كلمة المرور الجديدة',
    confirmPassword: 'تأكيد كلمة المرور',
    agreeTo: 'أوافق على',
    terms: 'شروط الخدمة',
    and: 'و',
    privacy: 'سياسة الخصوصية',
    or: 'أو'
  },
  placeholder: {
    name: 'ادخل اسمك الكامل',
    email: 'ادخل بريدك الإلكتروني',
    phone: 'ادخل رقم جوالك (مثال: +966569420800)',
    userName: 'اختر اسم مستخدم',
    password: 'ادخل كلمة المرور',
    confirmPassword: 'تأكيد كلمة المرور'
  },
  login: {
    title: 'تسجيل الدخول',
    success: 'تم تسجيل الدخول بنجاح',
    successDescription: 'تم تسجيل الدخول بنجاح.',
    subtitle: 'سجل الدخول إلى حسابك',
    welcome: 'مرحبًا بعودتك',
    emailPlaceholder: 'أدخل بريدك الإلكتروني',
    passwordPlaceholder: 'أدخل كلمة المرور',
    rememberMe: 'تذكرني',
    forgotPassword: 'نسيت كلمة المرور؟',
    submit: 'تسجيل الدخول',
    signingIn: 'جاري تسجيل الدخول...',
    noAccount: 'ليس لديك حساب؟',
    signUp: 'إنشاء حساب',
    continueWith: 'أو المتابعة باستخدام'
  },
  register: {
    title: 'إنشاء حساب',
    description: 'هل ترغب في أن تصبح مستخدمًا في {site_name} ؟ أنشئ حسابك',
    message: 'احصل على عمليات دفع أسرع، الوصول إلى سجل طلباتك، إنشاء قائمة أمنيات والانضمام إلى {site_name}.',
    success: 'تم إنشاء الحساب بنجاح',
    successDescription: 'تم إنشاء حسابك بنجاح. يمكنك الآن تسجيل الدخول.',
    subtitle: 'املأ بياناتك للبدء',
    submit: 'إنشاء حساب',
    creating: 'جاري إنشاء الحساب...',
    weak: 'ضعيف',
    fair: 'مقبول',
    good: 'جيد',
    strong: 'قوي',
    social: {
      google: 'المتابعة باستخدام Google',
      facebook: 'المتابعة باستخدام Facebook'
    }
  },
  verification: {
    title: 'أدخل رمز التحقق',
    subtitle: 'لقد أرسلنا رمزًا مكونًا من 6 أرقام إلى بريدك الإلكتروني',
    emailSentTo: '{email}. يرجى إدخاله أدناه للتحقق من هويتك.',
    codeLabel: 'رمز التحقق',
    expiresIn: 'تنتهي صلاحية الرمز في {time}',
    submit: 'تحقق من الرمز',
    resend: 'لم تستلم الرمز؟',
    resendLink: 'إعادة إرسال الرمز',
    backToLogin: 'العودة لتسجيل الدخول',
    success: 'تم التحقق بنجاح',
    successDescription: 'تم التحقق من بريدك الإلكتروني بنجاح.',
    resendSuccess: 'تم إعادة إرسال رمز التحقق',
    resendFailed: 'فشل في إعادة إرسال الرمز'
  },
  forgotPassword: {
    title: 'إعادة تعيين كلمة المرور',
    description: 'أدخل البريد الإلكتروني الذي سجلت به، وسنرسل لك تعليمات لإعادة تعيين كلمة المرور.',
    emailPlaceholder: 'بريدك الإلكتروني',
    submit: 'إرسال رابط إعادة التعيين',
    backToLogin: 'العودة لتسجيل الدخول',
    success: 'تم إرسال الرابط',
    successDescription: 'لقد أرسلنا رابط إعادة تعيين كلمة المرور إلى بريدك الإلكتروني.'
  },
  changePassword: {
    title: 'تغيير كلمة المرور',
    description: 'ادخل كلمة المرور الجديدة',
    success: 'تم إعادة تعيين كلمة المرور بنجاح',
    successDescription: 'تم إعادة تعيين كلمة المرور بنجاح. يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة.'
  },
  resetPassword: {
    title: 'إعادة تعيين كلمة المرور',
    subtitle: 'أدخل كلمة المرور الجديدة أدناه',
    submit: 'تغيير كلمة المرور',
    success: 'تم تغيير كلمة المرور بنجاح',
    successDescription: 'تم تغيير كلمة المرور الخاصة بك بنجاح. يمكنك الآن تسجيل الدخول.'
  },
  guest: {
    title: 'المتابعة كزائر',
    description: 'استمتع بتصفح سلس كزائر'
  },
  logout: 'تسجيل الخروج',
  rememberMe: 'تذكرني',
  continueAsGuest: 'المتابعة كزائر',
  continueAsRegistered: 'المتابعة كمستخدم مسجل',
  showPassword: 'إظهار كلمة المرور',
  hidePassword: 'إخفاء كلمة المرور',
  hasAccount: 'لديك حساب بالفعل؟',
  noAccount: 'ليس لديك حساب؟',
  layout: {
    welcomeTitle: 'إطلاق العنان للقدرات،',
    welcomeSubtitle: 'جلسة واحدة في كل مرة.',
    features: {
      insight: {
        title: 'رؤية معرفية عميقة',
        desc: 'تتبع الانتباه والجهد الذهني في الوقت الفعلي.'
      },
      monitoring: {
        title: 'مراقبة سلسة',
        desc: 'تكامل غير مزعج مع التكنولوجيا القابلة للارتداء.'
      },
      security: {
        title: 'أمن البيانات',
        desc: 'تشفير بمستوى المؤسسات لجميع البيانات العصبية.'
      },
      integration: {
        title: 'تكامل ذكي',
        desc: 'يتصل بسهولة مع أنظمة التعلم الحالية الخاصة بك.'
      }
    }
  }
}
